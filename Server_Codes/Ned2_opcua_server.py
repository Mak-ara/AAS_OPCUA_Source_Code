...
AAS_PATH  = "/home/niryo/niryo_robot_saved_files/NedRobot2_v1.aasx"
ROBOT_IP  = "127.0.0.1"
UPDATE_HZ = 1.0
...
,
    then stops the conveyor. Returns True if object was detected, False on ti
meout.
    """
    ...
    return [ua.Variant(bool(detected), ua.VariantType.Boolean)]
...
def _method_detect_object(robot,args):
    workspace_name = _get_arg(args, 0, "")
    shape_str      = str(_get_arg(args, 1, "ANY")).upper()
    color_str      = str(_get_arg(args, 2, "ANY")).upper()
    ...
    with ROBOT_LOCK:
        obj_found,pos_array,shape_ret,color_ret = robot.detect_object(workspace_name,shape=shape,color=color)

    shape_val = shape_ret.value if obj_found else ""
    color_val = color_ret.value if obj_found else ""

    print(f"  [VisionPick] obj_found={obj_found}, shape={shape_val}, color={color_val}")

    return [
        ua.Variant(bool(obj_found), ua.VariantType.Boolean),
        ua.Variant(shape_val, ua.VariantType.String),
        ua.Variant(color_val, ua.VariantType.String),
    ]
...
CAPABILITY_REGISTRY = {
    "move_pose": (
        ...
        [("x", ua.ObjectIds.Double), ("y", ua.ObjectIds.Double), ("z", ua.ObjectIds.Double),
         ("roll", ua.ObjectIds.Double), ("pitch", ua.ObjectIds.Double), ("yaw",ua.ObjectIds.Double)],
...
, ("direction", ua.ObjectIds.Double)],
    ...
    "vision_pick": (
        _method_vision_pick,
        [("workspace_name", ua.ObjectIds.String), ("height_offset", ua.ObjectIds.Double),
    ...
    "detect_object":(
        _method_detect_object,
        [("workspace_name", ua.ObjectIds.String),("shape", ua.ObjectIds.String), ("color", ua.ObjectIds.String)],
        [("ObjectFound", ua.ObjectIds.Boolean), ("Shape", ua.ObjectIds.String), ("Color", ua.ObjectIds.String)],
        ),
}
...
def live_update_loop(robot_ref, discovery, aas_data, aas_path, interval):
    ...
    sensor_entries = [(node, path) for node, path in discovery.bool_fields
                       if "sensor" in path[-1].lower() or "infrared" in " ".join(path).lower()]
